...
def process_all_csv_in_folder(folder_path, model):
    ...
    for filename in os.listdir(folder_path):
        ...
        if filename.endswith(".csv") and not filename.endswith("_csi_data.csv"):
            ...
            # calculate here the embeddings using the model already trained
            ...
            sequences = []
            sequences.append(torch.Tensor(phase_matrix_sanitize))
            print("heatmap type: ", type(heatmap_image))
            tensor = transform(heatmap_image)
            max_seq_len = 349
            phase_padded = pad_sequences(sequences, max_seq_len)
            with torch.no_grad():
                embedding = model(phase_padded[0].unsqueeze(0), tensor.unsqueeze(0))
            ...
            # save the heatmap
            heatmap_image.save("/Users/sebastiandinu/Library/Mobile Documents/com~apple~CloudDocs/01 Projects/Tesi/re-identification-csi/dataset_heatmap/" + folder_path.split("/")[-1] + "/" + filename[:-4] + ".png")
            ...
            process_all_csv_in_folder(file_path, model)
...
if __name__ == "__main__":
    ...
    # model using only vit_model
    # model = HeatmapViT(image_size=224, patch_size=16, num_classes=5)  # ViT model
    # load the model
    ...
    process_all_csv_in_folder("/Users/sebastiandinu/Library/Mobile Documents/com~apple~CloudDocs/01 Projects/Tesi/dataset", model) # change with your dataset's path
    ...
    torch.save(all_embeddings, "all_embeddings.pt") # save the batch matrix (35, max_len_seq, 64)
